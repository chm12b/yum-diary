...
type MenuLightboxProps = {
  ...
  onClose: () => void;
  /**
   * Optional delete handler (management context only). Deletes the photo at the
    ...
    >
      ...
      >
        <Image
          key={active.id}
          src={active.url}
          alt={`${restaurantName} 菜單 ${clamped + 1}`}
          fill
          sizes="100vw"
      ...
      {showToolbar ? (
        <div className="absolute inset-x-0 bottom-6 flex items-center px-6">
          {/* Left slot — delete stays pinned left */}
          ...
          {/* Center slot — page count stays horizontally centred */}
          <p className="flex-none text-sm font-medium text-white/90">
            {clamped + 1} / {total}
          </p>

          {/* Right slot — empty spacer balances the left slot width */}
